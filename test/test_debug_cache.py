...
async def test_debug_scan_disconnected_reports_error():
    ...
    result = await cache.debug_scan(match="gi:*")
...
async def test_debug_scan_truncates_large_values_and_respects_limit():
    ...
    capped = await cache.debug_scan(match="gi:*", limit=2, max_value_chars=10)
    assert capped["count"] == 2
    assert capped["truncated"] is True
    assert capped["entries"][0]["value"] == "x" * 10
    assert capped["entries"][0]["value_truncated"] is True
    assert capped["entries"][0]["value_chars"] == 50

    full = await cache.debug_scan(match="gi:*")
    assert full["count"] == 3
    assert full["truncated"] is False
    assert full["entries"][0]["value"] == "x" * 50
    assert "value_truncated" not in full["entries"][0]
...
def test_debug_cache_endpoint_dumps_when_flag_on():
    ...
    redis.debug_scan.assert_awaited_once_with(match="gi:*", limit=None)
